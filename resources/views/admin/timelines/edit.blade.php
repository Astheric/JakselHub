...
@section('title', 'Ubah Milestone Sejarah – Jaksel Hub')
@section('page_title', 'Ubah Milestone Sejarah')
...
@section('content')
    ...
    <div class="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm max-w-2xl">
        <form method="POST" action="{{ route('admin.timelines.update', $timeline->id) }}" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                ...
                <div class="space-y-2 sm:col-span-2">
                    ...
                    <input type="text" name="year" id="year" value="{{ old('year', $timeline->year) }}" required class="w-full bg-slate-50 border border-slate-200 text-slate-800 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-emerald-500 transition-colors">
                </div>

                <!-- Order -->
                <div class="space-y-2">
                    <label for="order" class="block text-xs font-bold uppercase tracking-wider text-slate-600">Urutan Tampil</label>
                    <input type="number" name="order" id="order" value="{{ old('order', $timeline->order) }}" required class="w-full bg-slate-50 border border-slate-200 text-slate-800 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-emerald-500 transition-colors">
                </div>
            </div>
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="title" id="title" value="{{ old('title', $timeline->title) }}" required class="w-full bg-slate-50 border border-slate-200 text-slate-800 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-emerald-500 transition-colors">
            </div>

            <!-- Description -->
            <div class="space-y-2">
                <label for="description" class="block text-xs font-bold uppercase tracking-wider text-slate-600">Deskripsi Linimasa</label>
                <textarea name="description" id="description" rows="5" required class="w-full bg-slate-50 border border-slate-200 text-slate-800 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-emerald-500 transition-colors">{{ old('description', $timeline->description) }}</textarea>
            </div>
                ...
                @if($timeline->image_path)
                    <div class="mb-3">
                        ...
                        <div class="w-40 h-28 bg-slate-100 rounded-xl overflow-hidden border border-slate-200">
                            <img src="{{ asset($timeline->image_path) }}" alt="{{ $timeline->title }}" class="w-full h-full object-cover">
                        </div>
                    </div>
                @endif
